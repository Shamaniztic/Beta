...
namespace TbsFramework.Units
{
    ...
    public class Unit : MonoBehaviour
    {
        ...
        public int TotalHitPoints
        {
            ...
            set
            {
                if (value < totalHitPoints)
                {
                    Debug.Log($"Attempt to reduce TotalHitPoints from {totalHitPoints} to {value}");
                    Debug.Log($"Reduction occurred in: {Environment.StackTrace}");
                }
        ...
        public void ResetTurn() => HasDoneTurn = false;
        ...
        public virtual void OnMouseDown()
        {
            ...
            {
                Debug.LogWarning("Unit.OnMouseDown: UnitClicked event is null");
            }
        }
    ...
    {
        if (PlayerNumber == 0) // Assuming player number 0 is the human player
        {
            if (actionMenu != null)
            {
                ...
                if (moveAbility != null)
                {
                    actionMenu.SetMoveAbility(moveAbility);
                    actionMenu.SetCellGrid(FindObjectOfType<CellGrid>());
